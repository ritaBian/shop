  ...
  <div class="page_body">
    <div class="house_swiper_border">
      ...
      <div class="pageIcon iconRight" @click="onStore">
        ...
        <div class="house_swiper_index" v-if="pageData.PICS!=undefined?pageData.PICS.length!=0:''">
          <div style="margin-left: 0.12rem;">{{current_house}}/{{pageData.PICS.length}}</div>
        </div>
    </div>
    <div class="house_content">
      <div class="house_title" v-if="pageData.PRO!=undefined">{{pageData.PRO.NAME}} {{pageData.PRO.NAME2}}</div>
      <div class="house_address" v-if="pageData.EXTRA!=undefined">领票地址: {{pageData.EXTRA.ADDRESS}}</div>
      ...
      <div class="house_flex" v-if="pageData.EXTRA!=undefined">
        <div class="house_flex_once" >适合人数：{{pageData.EXTRA.PERSON_COUNT}}</div>
        <div class="house_flex_once" >最少起订：{{pageData.EXTRA.MIN_COUNT}}</div>
        <div class="house_flex_once" style="width: 100%;">所属区域：{{pageData.EXTRA.PROVINCE_NAME}} {{pageData.EXTRA.CITY_NAME}} {{pageData.EXTRA.AREA_NAME}}</div>
        <div class="house_flex_once" v-if="pageData.EXTRA.SELL_POINT!=''">卖点：{{pageData.EXTRA.SELL_POINT}}</div>
      </div>
      ...
      <!--
      <div :class="['house_detail',height_house>106?'content_detail_line':'']" ref="house" v-if="pageData.PRO!=undefined" v-html="htmlList(pageData.PRO.MESS)" >
...
<script>
  ...
  export default {
        ...
        return function(html){
           ...
           if(html!=''&&html!=undefined){
             if(html.indexOf(`<img src="`)!=-1){
               ...
               return richtext
               // return 'dwaDjwlajdwklajdlkwajdwadDwad的味道快乐挖掘的快乐娃记录建档立卡我决定了卡机往来款大家啊网络科技了大口径walk都打完了看见了看见啦矿务局来我家埃里克大家洼漏孔的记录卡我就打我就快打完waDjwlajdwklajdlkwajdwadDwaDjwlajdwklajdlkwajdwadDwaDjwlajdwklajdlkwajdwadD'
             }
           }else{
             return ''
             // return 'dwaDjwlajdwklajdlkwajdwadDwad的味道快乐挖掘的快乐娃记录建档立卡我决定了卡机往来款大家啊网络科技了大口径walk都打完了看见了看见啦矿务局来我家埃里克大家洼漏孔的记录卡我就打我就快打完waDjwlajdwklajdlkwajdwadDwaDjwlajdwklajdlkwajdwadDwaDjwlajdwklajdlkwajdwadD'
    ...
    methods: {
      onTextDiv(){
        console.log(999999,this.$refs.content,this.$refs.content.offsetHeight)
...
<style scoped="scoped" src="../../css/shop_detail_eat.css">
